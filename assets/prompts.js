...
不要依賴系統是否能取得「上一次成功執行時間」，也不要自行擴大為最近數日、最近一週或其他歷史範圍。
...
Google Drive 先以本次時間範圍內新增或修改的會議紀錄作為候選，再實際讀取內容判斷是否屬於目前指定專案。

Gmail 只搜尋本次時間範圍內收到或寄出的、主旨包含「[LINE紀錄]」的郵件，再實際讀取正文判斷是否屬於目前指定專案。

不得因為本次週期沒有新資料，就重新引用、摘要或輸出前一週期已處理過的舊資料。

如果本次時間範圍內沒有任何新的有效工作紀錄，直接輸出：

「本次週期沒有新的工作記憶需要更新。」

此時不要重新整理既有工作記憶，也不要把上一週期的資料當成本次結果。
